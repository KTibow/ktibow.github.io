<script lang="ts">
  ...
  let { angleToLeft, angleToRight, angleToCenter } = $derived.by(() => {
    ...
    if (hoveredBumper) {
      ...
      const maybeAngleToLeft = angles.findLast((angle) => angle < angleToCenter);
      const maybeAngleToRight = angles.find((angle) => angle > angleToCenter);
...
<ul>
  <li>
    ...
    )}" relative to the uppermost point on the Roomba.
  </li>
  <li>
