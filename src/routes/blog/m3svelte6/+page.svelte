...
<ul>
  <li>
    ...
    bad and worked bad. Switch to the modern and clean LoadingIndicator, or if you know how long the
    operation is supposed to take, try CircularProgressEstimate, LinearProgressEstimate, or
...
<ul>
  <li>
    Dialog now asks you to make use of the native <code>closedby</code> to control closing, and
    detecting changes to <code>open</code> to detect closing.
    <small>Your editor should've warned you of this.</small>
  </li>
  <li>
  ...
  <li>
    Switch now requires your theme to have <code>on-on-primary</code>, a color roughly equivalent to
...
<h2>More info</h2>
<p>
  Recently, tokens were moved to a <code>@layer tokens</code> block, allowing you to change their
  priority and to use
...
<p>
  ...
  more predictable. The font and background variable defaults were recently moved to the top level, so
  it makes sense to put your overrides at the top level.
</p>
<p>
